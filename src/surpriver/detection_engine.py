...
class ArgChecker:
    ...
    def check_arugments(self):
        ...
        granularity_constraints_list_string = "".join(
            str(value) + "," for value in granularity_constraints_list
        ).strip(",")
        ...
        if is_test == 1 and future_bars < 2:
            print(
                "You want to test but the future bars are less than 2. "
                "That does not give us enough data to test the model properly. "
                "Please use a value larger than 2.\nExiting now..."
            )
            exit()
...
class Surpriver:
    def __init__(self):
        ...
        self.dataEngine = DataEngine(
            ...
            self.IS_SAVE_DICTIONARY,
            self.IS_LOAD_FROM_DICTIONARY,
            self.DATA_DICTIONARY_PATH,
            self.MINIMUM_VOLUME,
            self.IS_TEST,
    ...
    def calculate_volume_changes(self, historical_price):
        ...
        return (
            latest_data_point,
            self.parse_large_values(today_volume),
            self.parse_large_values(average_vol_last_five_days),
    ...
    def find_anomalies(self):
        ...
        for item in predictions_with_output_data[: self.TOP_PREDICTIONS_TO_PRINT]:
            ...
            if self.IS_TEST == 1 and len(future_price) < 5:
                print(
                    "No future data is present. Please make sure that you ran the prior command "
                    "with is_test enabled or disable that command now. Exiting now..."
                )
                exit()
            ...
            ) = self.calculate_volume_changes(historical_price)
            (
    ...
    def calculate_future_stats(self, predictions_with_output_data):
        ...
        anomalous_future_changes = np.mean(
            [
                future_change[x]
                for x in range(0, len(future_change))
                if anomalous_score[x] < 0
            ]
        )  # Anything less than 0 is considered anomalous
        ...
        anomalous_future_volatilities = np.mean(
            [
                future_volatilities[x]
                for x in range(0, len(future_volatilities))
                if anomalous_score[x] < 0
            ]
        )  # Anything less than 0 is considered anomalous
        ...
        anomalous_historical_volatilities = np.mean(
            [
                historical_volatilities[x]
                for x in range(0, len(historical_volatilities))
                if anomalous_score[x] < 0
            ]
        )  # Anything less than 0 is considered anomalous
        ...
        print("\n*************** Future Performance ***************")
        print(
            "Correlation between future absolute change vs anomalous score (lower is better, range = (-1, 1)): **%.2f**"
        ...
        colors = [
            ...
            for x in range(0, len(anomalous_score))
        ]
        anomalous_vs_normal = np.array(
            [1 if anomalous_score[x] < 0 else 0 for x in range(0, len(anomalous_score))]
        )
        plt.scatter(
            np.array(anomalous_score)[anomalous_vs_normal == 1],
            np.array(future_change)[anomalous_vs_normal == 1],
            marker="v",
            color="#c91414",
        )
        plt.scatter(
            np.array(anomalous_score)[anomalous_vs_normal == 0],
            np.array(future_change)[anomalous_vs_normal == 0],
            marker="P",
            color="#035AA6",
        )
