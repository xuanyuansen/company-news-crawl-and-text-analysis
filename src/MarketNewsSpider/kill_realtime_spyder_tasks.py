...
class KillPyTasks(object):
    def __init__(self):
        ...
        for _id in range(
            self.redis_client.llen(config.CACHE_RECORED_OPENED_PYTHON_PROGRAM_VAR)
        ):
            proc = self.get_python_process(
                param=self.redis_client.lindex(
                    config.CACHE_RECORED_OPENED_PYTHON_PROGRAM_VAR, _id
                ).decode()
            )
            for p in proc:
    ...
    def print_pid_info(process):
        logging.info(
            "{} | {} | {} -> killed ... ".format(
                process.Handle, process.Caption, process.CommandLine
            )
        )
