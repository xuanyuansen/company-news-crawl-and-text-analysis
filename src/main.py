...
jrj_spyder.get_historical_news(
    config.WEBSITES_LIST_TO_BE_CRAWLED_JRJ, start_date="2020-01-01"
)
...
gen_stock_news_db.get_all_news_about_specific_stock(
    config.DATABASE_NAME, config.COLLECTION_NAME_NBD
)
gen_stock_news_db.get_all_news_about_specific_stock(
    config.DATABASE_NAME, config.COLLECTION_NAME_JRJ
)
