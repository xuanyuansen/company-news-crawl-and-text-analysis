...
class ChanSourceDataObject(object):
    ...
    def __init__(self, level: str, data: list):
        ...
        self.macd_std: pd.DataFrame = pd.DataFrame()
        self.macd_std_cross: pd.DataFrame = pd.DataFrame()
        self.cross_list = list()

        if len(self.k_line_list) > 0:
            if not isinstance(self.k_line_list[0], KiLineObject):
                raise Exception(
                    "data type not right, type is {0}".format(type(data[0]))
                )
            # 取中间的date index
            ...
            # 处理占位符号以及准备数据
            for element in self.k_line_list:
            ...
            bi_res = from_point_to_bi(self.k_line_list, self.shape_2_bi_value_to_plot)
            ...
            if bi_res[0]:
                ...
                self.zhong_shu_list = from_xian_duan_to_zhong_shu(
                    self.merged_chan_line_list
                )
                ...
                for zs_element in self.zhong_shu_list:
                    ...
                    for idx in range(zs_element.start_index, zs_element.end_index - 1):
                        self.zhong_shu_upper_line_to_plot[
                            idx
                        ] = zs_element.min_max_value
                        self.zhong_shu_bottom_line_to_plot[
                            idx
                        ] = zs_element.max_low_value
    ...
    def gen_data_frame(self):
        ...
        date_middle_convert_index = pd.DatetimeIndex(
            [element.date[0] for element in self.k_line_list], name="Date"
        )

        self.data_to_plot_frame = pd.DataFrame(
            data=date_middle_convert, index=date_middle_convert_index
        )
        # get_volume_break_point
        # 这里新加了几个维度的数据，'average_volume'，'idx'，'break_point_of_buy'，'break_point_of_sell'
        # 这里的 'idx' 可以和中枢类里面的index对应
        get_res_success = inner_get_average_volume(
            self.data_to_plot_frame, level=self.k_line_level
        )
        ...
        # MACD > 0, 标准化到 1， MACD<0, 标准化到 -1
        self.macd_std = self.histogram.map(lambda x: 1 if x >= 0 else -1)
        # print('macd_std')
        # print(self.macd_std)
        # 前后2天 macd_std 相乘，如果等于-1，说明发生交叉，可能是金叉或者死叉；
        self.macd_std_cross = self.macd_std * self.macd_std.shift(1)
        # print('macd_std_cross')
        # print(self.macd_std_cross)

        # 这个计算不对
        for i in range(1, self.macd_std_cross.shape[0] - 1):
            if self.macd_std_cross[i] == -1 and self.macd_std[i] == 1:
                # red
                self.cross_list.append((self.macd_std.index[i], 1))
    ...
    def is_valid_buy_sell_point_on_k_line(self, level: str = "week"):
        ...
        if (
            last_2_macd[0] < h_param
            or last_2_macd[1] < h_param
            or last_2_signal[0] < h_param
            or last_2_signal[1] < h_param
        ):
        ...
        if "week" == level:
            ...
            if valid_ding_date is None or valid_di_date is None:
                ...
                return (
                    last_cross[1] == 1
                    and (
                        valid_di_date > valid_ding_date or distance >= distance_before
                    ),
                    last_cross,
                    valid_ding_date,
                    valid_di_date,
...
# volume 的计算采用左闭右开
def from_point_to_bi(data: list[KiLineObject], value_data):
    # new_line = []
    ...
    while idx_start < len(data) - 1:
        # up bi
        if data[idx_start].ding_di_to_bi == -1:
            ...
            if idx_end_j < len(data) - 1:
                ...
                idx_start += 1
        elif data[idx_start].ding_di_to_bi == 1:
            idx_end_j = idx_start + 1
            current_volume = 0
            while idx_end_j < len(data) - 1:
                if -1 == data[idx_end_j].ding_di_to_bi:
            ...
            if idx_end_j < len(data) - 1:
                ...
                try:
                    ...
                    print("value_data, idx_start", idx_start, data[idx_start].high)
                    print("data, idx_end_j", idx_end_j, data[idx_end_j])
                    print("value_data, idx_end_j", idx_end_j, data[idx_end_j].low)
                    raise Exception(e)
...
def debug(sub_origin_down_bi_list, standard_feature_line):
    ...
    print(
        "origin down bi list is :\n{0}".format(
            (
                ",  \n".join(
                    [
                        "index is: {0}, value is:{1}".format(ele[1], ele[0])
                        for ele in sub_origin_down_bi_list
                    ]
                )
            )
        )
...
def check_current_direction_line_with_other_direction_bi(
    ...
    if debug_f:
        print(
            "===============start check_current_direction_line_with_other_direction_bi======================="
        )
        print("start")
    # 第一个向下笔, 这里多加了1。
    start_idx = 1
    ...
    if debug_f:
        print(
            "origin length of feature line is {0}".format(len(sub_origin_down_bi_list))
        )

    if "up" == feature_line_direction:
        standard_feature_line = bi_inner_merge(
            [ele[0] for ele in sub_origin_down_bi_list], feature_line_direction
        )
    ...
    if debug_f:
        print(
            "standard length of feature line is {0}".format(len(standard_feature_line))
        )
            ...
            if standard_feature_line[idx_feature_line].high == max(
                ...
                if check_ding_di_shape_gap:
                    if (
                        ...
                        new_other_direction_feature_line = get_new_feature_line(
                            sub_origin_down_bi_list, bi_list, new_start_idx
                        )

                        has_other_shape = (
                            gap_between_first_second_element_then_shape_or_not(
                                new_other_direction_feature_line, feature_line_direction
                            )
                        )
                        if has_other_shape:
    ...
    if debug_f:
        print(
            "double check new stop point is {0}".format(
                new_stop_idx_in_origin_list_double_check
            )
        )
    if new_stop_idx_in_origin_list != new_stop_idx_in_origin_list_double_check:
        ...
        print("--------------------------------------------")
        print(
            "feature line ding shape index is {0} in origin bi list".format(
                new_stop_idx_in_origin_list
            )
        )
        print("--------------------------------------------")
        print(
            "new_stop_idx_in_origin_list_double_check index is {0} in origin bi list".format(
                new_stop_idx_in_origin_list_double_check
            )
        )
    ...
    if debug_f:
        print(
            "origin stop idx is {0}, new stop idx is {1}".format(
                stop_idx, new_stop_idx_in_origin_list
            )
        )
        print(
            "find done!-------------------------------------------------------------------------------\n\n\n"
...
def bi_to_line_inner_check(
    ...
    if len(chan_line_list) <= 0:
        if "up" == bi_list[start_idx].direction:
            # 如果第二个向上笔出问题，那么换方向
            if (
                start_idx + 1 < len(bi_list)
                and bi_list[start_idx].low >= bi_list[start_idx + 1].low
            ):
                return start_idx + 1
            ...
            if (
                start_idx + 1 < len(bi_list)
                and bi_list[start_idx].high <= bi_list[start_idx + 1].high
            ):
                return start_idx + 1
        else:
    ...
    if low_idx_j > len(bi_list) - 1:
        if len(chan_line_list) == 0:
            ...
            print("可能merge")
            if (
                chan_line_list[-1].direction == "up"
                and bi_list[start_idx].direction == "up"
            ):
                while start_idx <= len(bi_list) - 1:
                    if (
                        bi_list[start_idx].low >= chan_line_list[-1].low
                ...
                and bi_list[start_idx].direction == "down"
            ):
                while start_idx <= len(bi_list) - 1:
                    if (
                        bi_list[start_idx].low <= chan_line_list[-1].low
    ...
    while low_idx_j <= len(bi_list) - 1:
        ...
        if stop_idx != 0:
            if is_debug:
                print(
                    "direction {0} stop, idx {0}, stop idx is {1}".format(
                        direction, idx, stop_idx
                    )
                )
            # 转换成原始坐标，意味着出现了向下的笔破坏，这个时候考察特征序列
            actual_stop_index = check_current_direction_line_with_other_direction_bi(
                idx, bi_list[idx:], stop_idx, direction
            )
            ...
            # 还是按照笔破坏处理
                ...
                o_message = (
                    "direction {0}!!!笔破坏, start idx {0}, bi stop index{1}".format(
                ...
                if "up" == direction and bi_list[idx].low <= bi_list[stop_idx].high:
                    ...
                    o_message = (
                        "current line not right, direction is {0}, idx is {1}".format(
                            direction, start_idx
                        )
                    )
                    warnings.warn(o_message)
...
def from_bi_list_to_line(bi_list: list, is_debug: bool = False):
    ...
    while idx <= len(bi_list) - 1:
        if is_debug:
            print(
                "===============outer start check, index is {0}, bi is {1}.===================".format(
                    idx, bi_list[idx]
                )
            )
            print(
        ...
        out_idx = bi_to_line_inner_check(
...
def from_macd_seq_with_zhong_shu_2_buy_sell_point_list_afterwords(
    ...
    if len(zhong_shu_list) == 0:
        ...
        vol_b = {
            "up_cnt": volume_up.shape[0],
            "up_detail": volume_up["break_point_of_buy"].index,
            "down_cnt": volume_down.shape[0],
            "down_detail": volume_down["break_point_of_sell"].index,
        }
        print(vol_b)

        ret_current_type = "盘整"
        current_action = "判断是否盘整背驰"
        sub_action = last_zhong_shu_and_price(
            zhong_shu_list[-1], last_close_price, bi_all
        )
...
def last_zhong_shu_and_price(last_zhong_shu: ZhongShu, last_close_price, bi_all):
    ...
    if last_close_price > last_zhong_shu_middle_upper_price:
        ...
        print("判断回调是否破中枢")
        sub_xian_duan_min_vs = [
            ele.get_min()
            for ele in bi_all
            if ele.start_index >= last_zhong_shu.end_index
        ]
        xian_duan_min_value = min(sub_xian_duan_min_vs)

        if xian_duan_min_value > last_zhong_shu_middle_bottom_price:
            current_action = "不破中枢，距离中枢{0},多空分出胜负，脱离中枢价格{1}，可以选择涨幅小的买入".format(
                current_distance, break_ratio
            )
            # print(current_action)
        else:
    ...
    for idx in range(1, h_len):
        ...
        if next_macd < 0:
            ...
            if current_macd < next_macd:
                ...
                if buy_flag:
                    print(
                        "sell {0}, current_macd {1}".format(t_time[idx], current_macd)
                    )
                    buy_flag = False
...
def plot_with_mlf_v2(
    ...
    add_plot = [
        ...
        mpf.make_addplot(
            chan_data_object.histogram_positive,
            type="bar",
            width=0.7,
            panel=2,
            color="red",
        ...
        mpf.make_addplot(
            chan_data_object.macd,
            panel=2,
            color="fuchsia",
            ylim=(y_min - 0.1, y_max + 0.1),
        ),
        mpf.make_addplot(
            chan_data_object.signal, panel=2, color="b", ylim=(y_min - 0.1, y_max + 0.1)
        ),
        # BOLL线
    ...
    if len(chan_data_object.bi_list) > 0:
        add_plot += [
            # valid 顶底分型
            # ding red and di  green
            mpf.make_addplot(
                data["Valid_ding_to_draw"],
                scatter=True,
                markersize=25,
                marker="^",
                color="r",
            ...
            # ding red and di  green
            mpf.make_addplot(
                data["Ding_to_draw"], scatter=True, markersize=8, marker="^", color="r"
            ),
            mpf.make_addplot(
                data["Di_to_draw"], scatter=True, markersize=8, marker="v", color="g"
            ),
            # 笔
            # chan bi
            mpf.make_addplot(
    ...
    if data.loc[data.break_point_of_buy.notnull(), :].shape[0] > 0:
        add_plot.append(
            mpf.make_addplot(
                data["break_point_of_buy"] * max_boll,
                scatter=True,
                markersize=10,
                marker="^",
                color="r",
    ...
    if data.loc[data.break_point_of_sell.notnull(), :].shape[0] > 0:
        add_plot.append(
            mpf.make_addplot(
                data["break_point_of_sell"] * max_boll,
                scatter=True,
                markersize=10,
                marker="v",
                color="blue",
    ...
    my_style = mpf.make_mpf_style(
        marketcolors=my_color, gridaxis="both", gridstyle="-.", y_on_right=False
    )
