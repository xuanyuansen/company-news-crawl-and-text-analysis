...
def inner_get_average_volume(
    ...
    for index in t_price.index:
        # 获得当前日期向前推7天的日期
        first_date = map_idx_date.get(
            0
            if (int(t_price.loc[index]["idx"]) - range_of_days) < 0
            else int(t_price.loc[index]["idx"] - range_of_days)
        )
        # print(lst_date)
        # print(type(lst_date))
        # 左闭右开，所以正好是前7天的交易量
        volume_num = t_price.loc[
            (t_price.index >= first_date) & (t_price.index < index)
        ][volume_col_name]
        # print(num)
    ...
    t_price["break_point_of_buy"] = t_price.apply(
        lambda row: 1.0
        if (row[volume_col_name] > v_ratio * row["average_volume"])
        & ((row[close_col_name] - row[open_col_name]) >= 0)
        & (row["idx"] > range_of_days)
        else numpy.nan,
        axis=1,
    )

    t_price["break_point_of_sell"] = t_price.apply(
        lambda row: 1.0
        if (row[volume_col_name] > v_ratio * row["average_volume"])
        & ((row[close_col_name] - row[open_col_name]) < 0)
        & (row["idx"] > range_of_days)
...
def from_xian_duan_to_zhong_shu(input_lines: list):
    ...
    while idx_duan < line_cnt:
        if len(zhong_shu_list) == 0 and idx_duan + 2 < line_cnt:
            res = ZhongShu.from_duan_2_zhong_shu(
                input_lines[idx_duan],
