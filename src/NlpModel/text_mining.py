...
import NlpModel.text_processing as tp
...
class TextMining(object):
        ...
        for tk in token:
            ...
            if len(relevantStockCode) != 0:
                ...
                collection.update(
                    ...
                    {
                        "$set": {
                            "relevantStock": " ".join(relevantStockCodeDuplicateRemoval)
                        }
                    },
                )
        ...
        for tokens in token_list:
            ...
            for tk in tokens:
                if len(tk) >= 3 and tk in list(stock_basic_info.name):
                    relevantStockCode.append(
                        list(stock_basic_info[(stock_basic_info.name == tk)].code)[0]
                    )
            ...
            with open(
                ...
                for dbName, colName in dbColLst:
                    ...
                    print(
                        ...
                        + " news from "
                        + dbName
                        + " database to CSV file successfully ... "
                    )
                    ...
                    for _id in idLst:
                        ...
                        if keys.find("relevantStock") != -1:
                            if (
                                ...
                                character = self.judgeGoodOrBadNews(
                                    stockCode,
                                    collection.find_one({"_id": ObjectId(_id)})["date"]
                                    .split(" ")[0]
                                    .replace("-", ""),
                                    kwarg["judgeTerm"],
                                )
                                ...
                                data = {
                                    "Date": collection.find_one({"_id": ObjectId(_id)})[
                                        "date"
                                    ],
                                    "Address": collection.find_one(
                                        {"_id": ObjectId(_id)}
    ...
    def classifyHistoryStockNews(self, dbName, stockCode, **kwarg):
        ...
        if kwarg["renewDict"]:
            ...
            for _id in idLst:
                ...
                if collection.find_one({"_id": ObjectId(_id)})["Character"] == "利好":
                    ...
                    characters.append(0)
            self.tp.genDictionary(
                articles,
                saveDict=True,
                saveDictPath=self.DictPath
                + "\\"
                + stockCode
                + "\\"
                + stockCode
            ...
            self.DictPath + "\\" + stockCode + "\\" + stockCode + "_dict.dict"
        ) or not os.path.exists(
            self.DictPath + "\\" + stockCode + "\\" + stockCode + "_bowvec.mm"
        ):
            if not os.path.exists(self.DictPath + "\\" + stockCode):
                os.makedirs(self.DictPath + "\\" + stockCode)
            db = self._Conn[dbName]
            ...
            print(
                " [*] generate and save the dictionary and bow-vector successfully ... "
            )
        else:
            db = self._Conn[dbName]
            collection = db.get_collection(stockCode)
        ...
        bowvec = corpora.MmCorpus(
            self.DictPath + "\\" + stockCode + "\\" + stockCode + "_bowvec.mm"
        )
        print(" [*] load dictionary and bow-vector successfully ... ")
        _, modelVec = self.tp.CallTransformationModel(
            dictionary,
            bowvec,
            modelType=kwarg["modelType"],
            tfDim=kwarg["tfDim"],
            renewModel=kwarg["renewModel"],
            modelPath=self.DictPath + "\\" + stockCode + "\\",
        )
        ...
        if len(relevant_stock_list) != 0:
            ...
            for i, code_list in enumerate(relevant_stock_list):
                for code in code_list:
                    ...
                    print(
                        " * use historical news to build SVM model of " + code + " ... "
                    )
                    self.classifyHistoryStockNews(
                        "Stock_News",
                        code,
                        modelType="lda",
                        tfDim=tfDim,
                    ...
                    print(
                        " * extend latest bow-vector to historical bow-vector of "
                        + code
                        + " ... "
                    )
                    ...
                    _, NewmodelVec = self.tp.CallTransformationModel(
                        dictionary,
                        bowvec_all,
                        modelType="lda",
                        tfDim=200,
                    ...
                    print(
                        " * convert latest lda vector to CSR matrix of "
                        + code
                        + " ... "
                    )
    ...
    def SVMClassifier(
        self, train_X, train_Y, test_X, test_Y, tuned_parameters, scores, stockCode
    ):
        """SVM Classifier.
        ...
        for score in scores:
            if not os.path.exists(
                self.DictPath + "\\" + stockCode + "\\" + stockCode + "_svm.pkl"
            ):
                clf = GridSearchCV(
                    svm.SVC(), tuned_parameters, cv=5, scoring="%s_weighted" % score
        ...
        print(
            " [*] train_pred:",
            precise_train / len(train_Y),
            ", test_pred:",
            precise_test / len(test_pred),
        )
        print(" " + "-" * 50)
        self._precise = precise_test / len(test_pred)

    def RdForestClassifier(
        self, train_X, train_Y, test_X, test_Y, tuned_parameters, scores, stockCode
    ):
        ...
        for score in scores:
            if not os.path.exists(
                self.DictPath + "\\" + stockCode + "\\" + stockCode + "_rdf.pkl"
            ):
                clf = GridSearchCV(
