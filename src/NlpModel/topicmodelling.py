...
from NlpModel.classifier import Classifier
...
class TopicModelling(object):
    ...
    def create_dictionary(self, raw_documents_list, save_path=None, is_saved=False):
        """
        ...
        # 找到只出现一次的token
        once_items = [
            _dict[tokenid] for tokenid, docfreq in _dict.dfs.items() if docfreq == 1
        ]
        ...
        documents_token_list = [
            token_list for token_list in documents_token_list if (len(token_list) != 0)
        ]
        # 找到只出现一次的token对应的id
        ...
        if is_saved and save_path:
            ...
            logging.info(
                "new generated dictionary saved in path -> {} ...".format(save_path)
            )

        return _dict, documents_token_list

    def renew_dictionary(
        self, old_dict_path, new_raw_documents_list, new_dict_path=None, is_saved=False
    ):
        documents_token_list = []
        ...
        if is_saved:
            ...
            logging.info(
                "updated dictionary by another raw documents serialized in {} ... ".format(
                    old_dict_path
                )
            )
    ...
    def create_bag_of_word_representation(
        self,
        raw_documents_list,
        old_dict_path=None,
        new_dict_path=None,
        ...
        if old_dict_path:
            ...
            corpora_dictionary, documents_token_list = self.create_dictionary(
                raw_documents_list, save_path=new_dict_path, is_saved=is_saved_dict
            )
            end_time = time.time()
            logging.info(
                "there are {} mins spent to create a new dictionary ... ".format(
        ...
        start_time = time.time()
        bow_vector = [
            corpora_dictionary.doc2bow(doc_token) for doc_token in documents_token_list
        ]
        end_time = time.time()
        logging.info(
    ...
    @staticmethod
    def transform_vectorized_corpus(
        corpora_dictionary, bow_vector, model_type="lda", model_save_path=None
    ):
        ...
        if model_type == "lsi":
            ...
            model = models.LsiModel(
                tfidf_vector, id2word=corpora_dictionary, num_topics=config.TOPIC_NUMBER
            )  # 初始化模型
            ...
            model = models.LdaModel(
                bow_vector, id2word=corpora_dictionary, num_topics=config.TOPIC_NUMBER
            )  # 初始化模型
    ...
    def classify_stock_news(
        ...
        logging.info(
            "fetch symbol '{}' historical news with label '{}' from [DB:'{}' - COL:'{}'] ... ".format(
                collection_name, label_name, database_name, collection_name
            )
        )
        ...
        if not os.path.exists(ori_dict_path):
            if not os.path.exists(bowvec_save_path):
                ...
                logging.info(
                    "create dictionary of historical news, and serialized in path -> {} ... ".format(
                        ori_dict_path
                    )
                )
                logging.info(
                    "create bow-vector of historical news, and serialized in path -> {} ... ".format(
                        bowvec_save_path
                    )
                )
            else:
        ...
        ) = self.renew_dictionary(ori_dict_path, [unseen_raw_document], is_saved=True)
        end_time = time.time()
        logging.info(
            "renew dictionary with unseen news tokens, and serialized in path -> {}, "
            "which took {} mins ... ".format(
                ori_dict_path, (end_time - start_time) / 60
            )
        )
        ...
        if topic_model_type == "lsi":
            ...
            model = models.LsiModel(
                ...
                num_topics=config.TOPIC_NUMBER,
            )  # 初始化模型
            model_vector = model[
                updated_tfidf_model_vector
            ]  # type -> <gensim.interfaces.TransformedCorpus object>
        ...
        start_time = time.time()
        latest_matrix = corpus2dense(
            model_vector, num_terms=model_vector.obj.num_terms
        ).T
        end_time = time.time()
        logging.info(
            "transform {} model vector space to numpy.adarray, "
            "which took {} mins ... ".format(
                topic_model_type.upper(), (end_time - start_time) / 60
            )
        )
        ...
        clf = self.classifier.train(
            train_x, train_y, test_x, test_y, model_type=classifier_model
        )
        end_time = time.time()
        logging.info(
            "finished training by sklearn {} using latest {} model vector space, which took {} mins ... ".format(
                classifier_model.upper(),
...
if __name__ == "__main__":
    ...
    unseen_raw_documents_list = [
        "格隆汇6月23日丨平安银行(000001.SZ)公布，近日收到《中国银保监会关于平安银行变更注册资本\
    ...
    for unseen_doc in unseen_raw_documents_list:
        chn_label = topicmodelling.classify_stock_news(
            unseen_doc,
            database_name,
            collection_name,
        ...
        logging.info(
            "document '{}...' was classified with label '{}' for symbol {} ... ".format(
                unseen_doc[:20], chn_label, collection_name
            )
        )
