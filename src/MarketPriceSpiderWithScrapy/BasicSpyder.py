...
class Spyder(object):
    ...
    def insert_data_to_col_from_dataframe(
        self, col: pymongo.collection.Collection, symbol: str, data: DataFrame
    ):
        insert_cnt = 0
        for _idx in range(data.shape[0]):
            ...
            #print(_tmp_dict)
            id_md5 = hashlib.md5(
                ("{0} {1}".format(symbol, _tmp_dict["date"])).encode(encoding="utf-8")
            ).hexdigest()
            if col.find_one({"_id": id_md5}) is not None:
                self.logger.info("id already exist {0} {1}".format(id_md5, _tmp_dict))
                continue
            _tmp_dict["_id"] = id_md5
            _tmp_dict["date"] = _tmp_dict["date"].isoformat()
