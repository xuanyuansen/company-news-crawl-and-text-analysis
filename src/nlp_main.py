...
if __name__ == '__main__':
    ...
    text_score = info_extract.predict_score("◎记者 朱文彬 ○编辑 邱江上市公司遭实控人签署协议转让股权和出售" +
                                            "资产，作为重大事项，及时、准确地进行信息披露，本应是")
    ...
    count_v3 = CountVectorizer(vocabulary=info_extract.vocabulary, max_df=0.8, decode_error='ignore')
    counts_to_predict_data = count_v3.fit_transform(to_predict_features)
    tfidf_to_predict_data = info_extract.tfidf_transformer.fit(counts_to_predict_data).transform(counts_to_predict_data)
    ...
    for idx in range(0, len(predict_result)):
        print("----------------------------------------")
        print("----nb--------{0}---------{1}--------".format(predict_proba_nb_label[idx], predict_proba_nb[idx]))
