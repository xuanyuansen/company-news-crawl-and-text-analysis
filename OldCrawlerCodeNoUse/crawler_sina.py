...
class WebCrawlFromSina(object):
            ...
            if "class" in span.attrs:
                if span["class"] == ["date"] or span["class"] == ["time-source"]:
                    ...
                    for dt in string:
                        if dt.find("年") != -1:
                            date += (
                                dt.replace("年", "-").replace("月", "-").replace("日", " ")
                            )
                        elif dt.find(":") != -1:
                            date += dt
                    break
        ...
        if len(self.realtimeNewsURL) == 0:
            ...
            for a in a_list:
                if (
                    "href" in a.attrs
                    and a.string
                    and a["href"].find("http://finance.sina.com.cn/stock/s/") != -1
                ):
                    if a["href"] not in self._AddressLst:
                        ...
                        (
                            ...
                            stockCodeLst,
                            article,
                        ) = self.getUrlInfo(a["href"])
                        while article == "" and self.Prob >= 0.1:
                            self.Prob -= 0.1
                            (
                                summary,
                                keyWords,
            ...
            for a in a_list:
                if (
                    ...
                    if (
                        a["href"] not in self.realtimeNewsURL
                        and a["href"] not in self._AddressLst
                    ):
                        self.realtimeNewsURL.append(a["href"])
                        (
                            summary,
    ...
    def CrawlHistoryCompanyNews(self, startPage, endPage):
        ...
        if AddressLst == []:
            urls = []
            url_Part_1 = "https://finance.sina.com.cn/roll/index.d.html?cid=56592&page="
            url_Part_2 = ""
    ...
    def coroutine_run(self):
        ...
        for page_range in page_ranges_lst:
            jobs.append(
                gevent.spawn(self.CrawlHistoryCompanyNews, page_range[0], page_range[1])
            )
        gevent.joinall(jobs)
    ...
    def classifyRealtimeStockNews(self):
        ...
        while True:
            ...
            doc_list = self.CrawlRealtimeCompanyNews(
