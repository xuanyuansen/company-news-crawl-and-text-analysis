...
from crawler_jrj import WebCrawlFromjrj
...
def crawlers(web):
    if web == "sina":
        ...
        web_crawl_obj = WebCrawlFromstcn(
            IP="localhost",
            PORT=27017,
            ThreadsNum=4,
            dbName="Stcn_Stock",
...
if __name__ == "__main__":
    ...
    while Idx < len(codeLst):
        ...
        for stockcode in codeLst[Idx : Idx + Range]:
            thread = threading.Thread(
                target=text_mining_obj.getNewsOfSpecificStock,
                args=(
                    [
                        ("NBD_Stock", "nbd_news_company"),
                ...
                kwargs={
                    "export": ["database", "Stock_News", stockcode],
                    "judgeTerm": 3,
                },
            )
    ...
    for stockcode in codeLst[Idx:]:
        thread = threading.Thread(
            ...
            kwargs={"export": ["database", "Stock_News", stockcode], "judgeTerm": 3},
        )
        thread_lst.append(thread)
    for thread in thread_lst:
        thread.start()
    ...
    with futures.ThreadPoolExecutor(max_workers=4) as executor:
        future_to_url = {
            executor.submit(crawlers, param): ind for ind, param in enumerate(web_list)
        }
