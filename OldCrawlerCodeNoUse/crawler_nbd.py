...
class WebCrawlFromNBD(object):
                    ...
                    for a in a_list:
                        if (
                            ...
                            if date == "" or article == "":
                                ...
                                self.CrawledUrlsID.append(
                                    int(url_list[0].split("/")[-1])
                                )
    ...
    def multi_threads_run(self):
        ...
        for page_range in page_ranges_lst:
            thread = threading.Thread(
                target=self.CrawlCompanyNews, args=(page_range[0], page_range[1])
            )
    ...
    def coroutine_run(self):
        ...
        for page_range in page_ranges_lst:
            jobs.append(
                gevent.spawn(self.CrawlCompanyNews, page_range[0], page_range[1])
            )
        gevent.joinall(jobs)
        return self.url_lst_withoutNews
