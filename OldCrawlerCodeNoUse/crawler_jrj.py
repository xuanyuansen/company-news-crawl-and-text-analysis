...
class WebCrawlFromjrj(object):
            ...
            for url, specificDate in urlsAndDates:
                ...
                for a in a_list:
                    if (
                        ...
                        and a["href"].find(
                            "/"
                            + specificDate.replace("-", "")[0:4]
                            + "/"
                            + specificDate.replace("-", "")[4:6]
                            + "/"
                        )
                        != -1
                    ):
                        if a["href"] not in self._AddressLst:
                            ...
                            date, article, NotFoundPage = self.getUrlInfo(
                                a["href"], specificDate
                            )
                            while (
                                article == "" and self.Prob >= 0.1 and not NotFoundPage
                            ):
                                self.Prob -= 0.1
                                date, article, NotFoundPage = self.getUrlInfo(
                                    a["href"], specificDate
            ...
            for url, specificDate in urlsAndDates:
                ...
                for a in a_list:
                    if (
                        ...
                        if (
                            a["href"] not in self._AddressLst
                            and a["href"] not in self.realtimeNewsURL
                        ):
                            self.realtimeNewsURL.append(a["href"])
                            date, article, NotFoundPage = self.getUrlInfo(
                                a["href"], specificDate
            ...
            for url, specificDate in urlsAndDates:
                ...
                for a in a_list:
                    if (
                        ...
                        while article == "" and self.Prob >= 0.1 and not NotFoundPage:
                            self.Prob -= 0.1
                            date, article, NotFoundPage = self.getUrlInfo(
                                a["href"], specificDate
                            )
    ...
    def StockCodeDuplicateRemoval(self):
        ...
        for _id in idLst:
            ...
            if "relevantStock" in data.keys():
                relevantStock = collection.find_one({"_id": ObjectId(_id)})[
                    "relevantStock"
                ]
                if len(relevantStock) > 1:
                    ...
                    collection.update(
                        {"_id": _id},
                        {
                            "$set": {
                                "relevantStock": " ".join(
                                    relevantStockCodeDuplicateRemoval
                                )
                            }
                        },
                    )
    ...
    def multi_threads_run(self, **kwarg):
        ...
        with futures.ThreadPoolExecutor(max_workers=self.ThreadsNum) as executor:
            future_to_url = {
                executor.submit(self.CrawlHistoryCompanyNews, datelst): ind
                for ind, datelst in enumerate(dateLst)
            }
